<<<<<<< HEAD
const fs = require('fs')
const {fullShowFile} = require('./fullShowFile')
...
let fastArrayLength = Math.pow(1024, 2)*64
...
for(let i=0; i<fileSize; i+=fastArrayLength){
    ...
    if(i+fastArrayLength>fileSize){
        //console.log('Вызов')
        fastBuffer = Buffer.alloc(fileSize-1)
        fs.readSync(fd_a, fastBuffer, 0, fileSize-i, i)
        let arr = new Uint8Array(fastBuffer)
        //console.log(arr)
        arr.sort(compareNumeric)
        fs.writeSync(fd_a, new Buffer.from(arr), 0, fileSize-i, i)
        break
    }
    else{
        fastBuffer = Buffer.alloc(fastArrayLength)
        fs.readSync(fd_a, fastBuffer, 0, fastArrayLength, i)
    }
    let arr = new Uint8Array(fastBuffer)
    //console.log(arr)
    arr.sort(compareNumeric)
    fs.writeSync(fd_a, new Buffer.from(arr), 0, fastArrayLength, i)
}
//showFile(fd_a)
//fullShowFile('a_file.bin')

console.log('n', Math.ceil(Math.log(fileSize)/Math.log(2)))
console.log('n2', Math.log(fileSize)/Math.log(2))
console.log(fileSize)
//Сортировка
for(let n=Math.log(fastArrayLength)/Math.log(2)+1; n<=Math.ceil(Math.round(Math.log(fileSize)/Math.log(2) * 10000) / 10000); n++){
    ...
    //fullShowFile('b_file.bin')
    //fullShowFile('c_file.bin')
...
function whatContains(arr){
    ...
    return str+']'
>>>>>>> b76f2b045cfaee45cd2144abdeff700394af84e5
}